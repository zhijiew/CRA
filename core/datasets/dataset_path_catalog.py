...
from .cityscapes_self_distill_cra import cityscapesSelfDistillDataSetCra
...
class DatasetCatalog(object):
    ...
    DATASETS = {
        ...
        "cityscapes_self_distill_train": {
            "data_dir": "cityscapes",
            "data_list": "cityscapes_train_list.txt",
            "label_dir": "cityscapes/default_soft_labels/inference/cityscapes_train"
#             "label_dir": "cityscapes/soft_labels/inference/cityscapes_train"
    ...
    @staticmethod
    def get(name, mode, num_classes, max_iters=None, transform=None, label_dir=None, flag=None):
        if "gta5" in name:
            ...
            if 'distill' in name:
                if flag == 'sd':
                    if 'home' in data_dir:
                        args['root'] = data_dir
                    args['label_dir'] = os.path.join(data_dir, label_dir)
                    ...
                    if 'home' in data_dir:
                        args['root'] = data_dir
                    args['label_dir'] = os.path.join(data_dir, label_dir)
                    return cityscapesSelfDistillDataSetCra(args["root"], args["data_list"], args['label_dir'], max_iters=max_iters, num_classes=num_classes, split=mode, transform=transform)
                else:
                    if 'home' in data_dir:
                        args['root'] = data_dir
