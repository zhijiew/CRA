    ...
    def __init__(
        ...
        if max_iters is not None:
            ...
            if self.NUM_CLASS == 19:
                for i in range(self.NUM_CLASS - 3):
                    ind[i] = 0
            else:
                for i in range(self.NUM_CLASS):
                    ind[i] = 0
            for e in range(int(max_iters/SUB_EPOCH_SIZE)+1):
                if self.NUM_CLASS == 19:
                    cur_class_dist = np.zeros(self.NUM_CLASS - 3)
                else:
                ...
                for i in range(SUB_EPOCH_SIZE):
                    ...
                    w = w/w.sum()
                    if self.NUM_CLASS == 19:
                        c = np.random.choice(self.NUM_CLASS - 3, p=w)
                    else:
            ...
            self.data_list.append(
                {
                    ...
                    "label": os.path.join(self.data_root, "RAND_CITYSCAPES/parsed_LABELS/%s" % name),
                    "name": name,
                }
            )
...
#         label = np.asarray(imageio.imread(datafiles["label"], format='PNG-FI'))[:,:,0]  # uint16
        label = np.asarray(imageio.imread(datafiles["label"], format='PNG-FI'))
        name = datafiles["name"]
