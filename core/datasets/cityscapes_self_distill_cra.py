...
class cityscapesSelfDistillDataSetCra(data.Dataset):
    def __init__(
        self,
        data_root,
        data_list,
            ...
            self.data_list.append(
                {
                    ...
                    "entropy": os.path.join(
                        self.label_dir,
                        name.split('/')[1].replace('png', 'npy'),
                    ),
                    "name": name,
                }
            )
    ...
    def __getitem__(self, index):
        ...
        entropy = np.load(datafiles["entropy"])[0][0]
        name = datafiles["name"]
        ...
        if self.transform is not None:
            image, label, entropy = self.transform(image, label, entropy)
        return image, label, entropy[0], name
